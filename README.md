...
在深度学习框架中，这其实就意味着，表征物理动作的物理运动指令向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]\in\mathbb{R}^A) 随时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的变化是平缓的，不会发生值的突变。<br>
...
无论如何，![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]) 都绝对不能直接基于脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{\left(L+1\right)}[\uptau]) 产生，这是非常危险的。<br>
...
<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\begin{align*}\mathbf{u}^{\left(L+1\right)}[\uptau]&=\mathbf{u}^{\left(L+1\right)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&=\mathbf{u}^{\left(L+1\right)}\left[\uptau-2\right]+\mathbf{U}^{(L)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&=\mathbf{u}^{\left(L+1\right)}\left[\uptau-3\right]+\mathbf{U}^{(L)}\left[\uptau-2\right]+\mathbf{U}^{(L)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&\cdots\\&=\mathbf{u}^{\left(L+1\right)}\left[-1\right]+\mathbf{U}^{(L)}[0]+\cdots+\mathbf{U}^{(L)}\left[\uptau-2\right]+\mathbf{U}^{(L)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&=\mathbf{u}^{\left(L+1\right)}\left[T-1\right]_{t-1\sim{}t}+\sum_{\uptau^\prime=0}^{\uptau}\mathbf{U}^{(L)}\left[\uptau^\prime\right],\end{align*}">
</p>
...
具备函数平滑性的 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]) 完美适配机器人的连续运动控制。<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]) 采用 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]) 后，机器人的运动学急动度可长期保持低位水平，机器人的动作将表现出良好的连贯性和协调性。<br>
原论文作者团队指出：<br>
“将脉冲神经网络应用于机器人技术的一个关键挑战在于弥合离散、随机的脉冲与关节驱动的连续域之间的差距。我们通过在输出层实现连续积分协议来解决这一问题。与经历快速复极化（重置）的隐藏中间神经元不同，输出运动神经元被配置为非重置积分器（non-resetting integrators）。它们在模拟窗口上持续累积突触输入而不释放其电位。”<br>
“该机制服务于两个生理功能：”
