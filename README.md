...
可以将 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(0)}[\uptau]) 另记为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)) ，其中 ![](https://latex.codecogs.com/svg.latex?\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)) 指的就是 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}) 线性转换为输入电压 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]) 的结果，即
...
本质上表征了LIF动力学随时间步的推进而演化的过程，包括“膜电位的衰减”、“外部电压的输入”、“膜电位的软重置/阈值电位的延迟扣除”这三大并行的部分，那么，![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) 在各个时间步下的值肯定是不同的，而且存在明确的时序先后依赖关系————后一时间步的量的计算显式地依赖于前一时间步的量的已知值，只有提前算出前一时间步的量，才能开始计算后一时间步的量。<br>
