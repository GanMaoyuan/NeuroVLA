...
很明显，它就是一种右连续约定的、向量形式的Heaviside阶跃函数，各分量的值被独立地判定为 ![](https://latex.codecogs.com/svg.latex?1) 或 ![](https://latex.codecogs.com/svg.latex?0) 。<br>
由于这类函数在零点处不可导，所以这类函数具备不可微性（non-differentiability），也就是说，这类函数对自变量的导数是不可解的。在深度学习的梯度下降（Gradient Descent）训练框架中，这意味着从损失函数到部分可学习参数（笔者将其称为“下游可学习参数”）的梯度链（梯度流）将在该函数处发生“断裂”，直接使得这部分下游参数的优化变得不可行，端到端（end-to-end）的梯度下降优化也就随之变得不可行。<br>
