...
VLM采用Transformer架构，其编码器（encoder）有 ![](https://latex.codecogs.com/svg.latex?N) 个隐藏层（hidden layers），每层内部都会进行自注意力（Self Attention）计算以及非线性前馈网络（Feed-Forward Network，FFN）计算，一般还会经过一次残差跳跃连接（深层神经网络的标准操作），最终输出一个隐藏状态 ![](https://latex.codecogs.com/svg.latex?h_t\in\mathbb{R}^{S\times{}D}) 。<br>
...
传感器的采样频率为 ![](https://latex.codecogs.com/svg.latex?f) ，即 ![](https://latex.codecogs.com/svg.latex?1) 秒钟之内总共进行 ![](https://latex.codecogs.com/svg.latex?f) 次采样（例如 ![](https://latex.codecogs.com/svg.latex?f=50\text{\space}\mathrm{Hz}) ）。<br>
...
应注意，实际的生理过程远远比这复杂，这仅仅是适配建模实现的必要简化。本篇解读笔记聚焦于具身智能工程实现技术NeuroVLA，不对精细的神经科学过程展开冗余论述。

## 2、输入电流积分 ![](https://latex.codecogs.com/svg.latex?I_{\text{in}}\left(\uptau\Delta{}t\right)=\sum_jI_{ij}\cdot{}s_j^{\left(l-1\right)}\left(\uptau\Delta{}t\right))
...
它就是“Leaky Integrate-and-Fire”当中“Integrate”（积分）一词的实质。

## 3、![](https://latex.codecogs.com/svg.latex?I_{\text{in}}\left(\uptau\Delta{}t\right)) 的去向
...
## 4、普适情形下的“流过电阻的电流” ![](https://latex.codecogs.com/svg.latex?I_R) 和“充电电流” ![](https://latex.codecogs.com/svg.latex?I_C) 的表达式
...
若将“流过电阻的电流”特意记为 ![](https://latex.codecogs.com/svg.latex?I_R) ，则有
...
其中 ![](https://latex.codecogs.com/svg.latex?dQ/dt) 即为电流 ![](https://latex.codecogs.com/svg.latex?I) 的定义式。因此，若将“充电电流”特意记为 ![](https://latex.codecogs.com/svg.latex?I_C) ，则有

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?I_C=C\cdot\frac{dU}{dt}.">
</p>

## 5、关于膜内电位 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}(t)) 的方程

综上所述，针对当前时刻 ![](https://latex.codecogs.com/svg.latex?t=\uptau\Delta{}t) 下的第 ![](https://latex.codecogs.com/svg.latex?l) 层第 ![](https://latex.codecogs.com/svg.latex?i) 个神经元，从基本关系式
...
这个方程。其中 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}(t)) 表示膜内电位，同时膜外电位定义为 ![](https://latex.codecogs.com/svg.latex?0)（可以认为膜外是“接地”的），因此 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}(t)) 还同时表示膜内、膜外的电位差（电压）。<br>
根据这个方程，可以推导出 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}(t)) 在离散时间步情形下的前向欧拉方法近似解（迭代式）。后一章将展示详细的推导过程。
<br><br><br><br><br><br><br><br><br><br>

# 脊髓模块——用前向欧拉方法近似求解 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}(t))
...
因此，该常微分方程同时属于非齐次常微分方程。

## 2、前向欧拉方法的介绍

前向欧拉方法（Forward Euler Method）用于常微分方程数值求解（未知函数 ![](https://latex.codecogs.com/svg.latex?y) 的函数值求解），属于显式单步法。其本质在于————用前一时间步的导数（斜率）进行“直线外推”，来估计当前时间步的函数值。在几何上，这相当于用一系列特别短小的折线（在时间轴上的投影长度仅仅等于单个离散（discrete）时间步长 ![](https://latex.codecogs.com/svg.latex?\Delta{}t) ）来逼近真实的、连续的（continuous）解曲线。这注定了其结构性的、不可消灭的数值误差。<br>
...
其中 ![](https://latex.codecogs.com/svg.latex?\beta) 不为 ![](https://latex.codecogs.com/svg.latex?0) 时，它就是非齐次项（既可以是常数，也可以是关于 ![](https://latex.codecogs.com/svg.latex?x) 的另一个无关函数）。![](https://latex.codecogs.com/svg.latex?\beta) 为 ![](https://latex.codecogs.com/svg.latex?0) 时，这个一阶线性常微分方程就是齐次的。<br>
...
前向欧拉方法是数值估计方法，而非数值精确解析方法，产生的数值误差是结构性的。以下对前向欧拉方法的数值稳定性展开论述。
...
它就是 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}(t)) 在离散时间步情形下的前向欧拉方法近似解（迭代式）。

## 5、来自笔者的提醒

需要提醒以下三点。着重提醒第一点。

### 5.1、第一点（尤其重要，容易混淆，务必区分）
...
这是**完全错误**的，根本没有任何依据可言。作为（相对于 ![](https://latex.codecogs.com/svg.latex?y=u_i^{(l)}(t)) 而言的）常数的
...
![](https://latex.codecogs.com/svg.latex?\gamma\in\left(0,1\right)) 可称为“膜衰减因子”（membrane decay factor）。

### 5.3、第三点

原论文中 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}\left[\uptau-1\right]) 的系数符号为 ![](https://latex.codecogs.com/svg.latex?\beta) 。为避免与本篇解读笔记的前文 ![](https://latex.codecogs.com/svg.latex?f(y)=\alpha{}y+\beta) 所使用的 ![](https://latex.codecogs.com/svg.latex?\beta) 产生符号冲突，笔者在本篇解读笔记中将 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}\left[\uptau-1\right]) 的系数符号设置为 ![](https://latex.codecogs.com/svg.latex?\gamma) ，与原论文中 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}\left[\uptau-1\right]) 的系数符号 ![](https://latex.codecogs.com/svg.latex?\beta) 没有本质区别。
<br><br><br><br><br><br><br><br><br><br>

# 脊髓模块——LIF动力学的进一步展开

## 1、可能需要被择机扣除的量 ![](https://latex.codecogs.com/svg.latex?-s_i^{(l)}[\uptau]\cdot\theta)

生理上，在一般情况下，膜内电位达到一定阈值后，会通过某种生理机制进行“重置”（reset），同时向其他神经元发放脉冲（即“Leaky Integrate-and-Fire”中的“Fire”）。在不考虑诸多实际生理因素的理想化计算模型中，“膜内电位达到一定阈值”可视为“向其他神经元发放脉冲”的充分必要条件。<br>
...
可能需要被择机扣除的量可以表示为 ![](https://latex.codecogs.com/svg.latex?-s_i^{(l)}[\uptau]\cdot\theta) 。在 ![](https://latex.codecogs.com/svg.latex?s_i^{(l)}[\uptau]=0) 时它等于 ![](https://latex.codecogs.com/svg.latex?0)（表示无需扣除），在 ![](https://latex.codecogs.com/svg.latex?s_i^{(l)}[\uptau]=1) 时它等于 ![](https://latex.codecogs.com/svg.latex?-\theta) 。

## 2、扣除操作的延迟性

实际上，前文所提到的 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}[\uptau]) 在严格意义上指的是————在时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 下，对“是否发放脉冲（是否达到阈值）”进行判定**之前**的膜内电位。这就必然限定了一种明确的数值先后依赖关系：![](https://latex.codecogs.com/svg.latex?s_i^{(l)}[\uptau]) 值的确定依赖于 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}[\uptau]) 值的确定。只有先得出 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}[\uptau]) 的值，才能紧随其后判定 ![](https://latex.codecogs.com/svg.latex?s_i^{(l)}[\uptau]) 的值。<br>
...
完整的 ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}[\uptau]) “软重置版”迭代式即为
...
表示突触权重（synaptic weight），![](https://latex.codecogs.com/svg.latex?\theta) 表示阈值电位。
...
“ ![](https://latex.codecogs.com/svg.latex?u_i^{(l)}\left[\uptau-1\right]) 项代表从过往片刻携带过来的残余电位（residual potential），这建立了一个连续的时序上下文（continuous temporal context）。”
<br><br><br><br><br><br><br><br><br><br>

# 脊髓模块——脉冲残差网络的 ![](https://latex.codecogs.com/svg.latex?l=0) 层

在小脑模块输出第 ![](https://latex.codecogs.com/svg.latex?I) 轮（最后一轮）迭代循环优化所产生的 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}^{(I)}\in\mathbb{R}^K) 之后，将其输入到脉冲残差网络（Spiking Residual Network）的 ![](https://latex.codecogs.com/svg.latex?l=0) 层（小脑-脊髓交界面数据转换接口层）。<br>
在后文中，![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}^{(I)}) 简记为 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}) 。

## 1、将小脑模块输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}\in\mathbb{R}^K) 转换为输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]\in\mathbb{R}^N)

### 1.1、输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]\in\mathbb{R}^N)

对 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}\in\mathbb{R}^K) 进行线性变换（Linear Transformation），将其转换为即将提供给最低层（ ![](https://latex.codecogs.com/svg.latex?l=0) 层）神经元的“输入电压向量”（用于使各神经元膜电位升高，对应于LIF动力学的膜电位迭代式中的 ![](https://latex.codecogs.com/svg.latex?+\sum{}w_{ij}\cdot{}s_j^{\left(l-1\right)}[\uptau]) 贡献项），即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]=W^{\left(-1\right)}\cdot\mathbf{z}_{\text{mod}}+b^{\left(-1\right)}\in\mathbb{R}^N,">
</p>

其中 ![](https://latex.codecogs.com/svg.latex?W^{\left(-1\right)}\in\mathbb{R}^{N\times{}K},b^{\left(-1\right)}\in\mathbb{R}^N) 属于可学习参数，![](https://latex.codecogs.com/svg.latex?N) 表示单层神经元（Neurons）个数（即脉冲残差网络的“宽度”）。

### 1.2、![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]\in\mathbb{R}^N) 的常数性

![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}) 是关于时刻 ![](https://latex.codecogs.com/svg.latex?t) 的函数向量（即 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}(t)) ，每一个时刻 ![](https://latex.codecogs.com/svg.latex?t) 单独产生一个 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}) ），但它在更细粒度的、处于时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个离散时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1)（分别对应时刻 ![](https://latex.codecogs.com/svg.latex?t,t+\Delta{}t,t+2\Delta{}t,\cdots,t+\left(T-1\right)\Delta{}t) ，时刻 ![](https://latex.codecogs.com/svg.latex?t+1) 实际上就是 ![](https://latex.codecogs.com/svg.latex?t+T\Delta{}t) ）下是保持不变的。<br>
换句话说，![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 保持通用，对它们而言属于常数向量。<br>
因此，作为线性变换结果的 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 同样保持为常数向量。

## 2、用输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]\in\mathbb{R}^N) “激发”膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(0)}[\uptau]\in\mathbb{R}^N) 及其配套的脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(0)}[\uptau]\in\mathbb{R}^N)

### 2.1、LIF动力学膜电位迭代式在脉冲残差网络中的实现形式
...
<p align="center">
<img src="https://latex.codecogs.com/svg.latex?u_i^{(l)}[\uptau]=\gamma\cdot{}u_i^{(l)}\left[\uptau-1\right]+U_i^{\left(l-1\right)}[\uptau]-s_i^{(l)}\left[\uptau-1\right]\cdot\theta,">
</p>

其向量形式即为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{(l)}[\uptau]=\gamma\cdot{}\mathbf{u}^{(l)}\left[\uptau-1\right]+\mathbf{U}^{\left(l-1\right)}[\uptau]-\theta\mathbf{s}^{(l)}\left[\uptau-1\right]\in\mathbb{R}^N,">
</p>

也就是说，求和项 ![](https://latex.codecogs.com/svg.latex?+\sum{}w_{ij}\cdot{}s_j^{\left(l-1\right)}[\uptau]) 整体被替代为 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) 向量的单个分量 ![](https://latex.codecogs.com/svg.latex?U_i^{\left(l-1\right)}[\uptau]) ，简化工程实现。

### 2.2、![](https://latex.codecogs.com/svg.latex?\uptau=0) 时的边界情况

针对 ![](https://latex.codecogs.com/svg.latex?\uptau=0) 时的边界情况

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{(l)}[0]=\gamma\cdot{}\mathbf{u}^{(l)}\left[-1\right]+\mathbf{U}^{\left(l-1\right)}[0]-\theta\mathbf{s}^{(l)}\left[-1\right]\in\mathbb{R}^N,">
</p>

“时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=-1) ”实际上指的是时刻 ![](https://latex.codecogs.com/svg.latex?t-1) 与 ![](https://latex.codecogs.com/svg.latex?t) 之间的时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=T-1) 。因此，“时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=-1) ”的膜电位向量和脉冲向量实际上都是上一时刻的时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=T-1) 下就算好的、可以直接用的已知量，可分别记为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\begin{align*}\mathbf{u}^{(l)}\left[-1\right]&=\mathbf{u}^{(l)}\left[T-1\right]_{t-1\sim{}t}\in\mathbb{R}^N\\\mathbf{s}^{(l)}\left[-1\right]&=\mathbf{s}^{(l)}\left[T-1\right]_{t-1\sim{}t}\in\mathbb{R}^N\end{align*}">
</p>

网络的各个层 ![](https://latex.codecogs.com/svg.latex?l=0,1,2,\cdots,L,L+1) 均遵循这种已知量沿用逻辑，这对网络的连续时序上下文的维护至关重要。<br>
不过，如果当前时刻为 ![](https://latex.codecogs.com/svg.latex?t=0) ，也就是“系统刚刚开机”，并没有“前一时刻已知量”可供沿用，则不妨将其初始化为零向量，以供后续计算，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\begin{align*}\mathbf{u}^{(l)}\left[-1\right]_{t=0\sim{}t=1}&=\mathbf{0}\in\mathbb{R}^N\\{}\mathbf{s}^{(l)}\left[-1\right]_{t=0\sim{}t=1}&=\mathbf{0}\in\mathbb{R}^N\end{align*}">
</p>

### 2.3、膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(0)}[\uptau]\in\mathbb{R}^N)

总之，![](https://latex.codecogs.com/svg.latex?l=0) 层的膜电位向量为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{(0)}[\uptau]=\gamma\cdot{}\mathbf{u}^{(0)}\left[\uptau-1\right]+\mathbf{U}^{\left(-1\right)}[\uptau]-\theta\mathbf{s}^{(0)}\left[\uptau-1\right]\in\mathbb{R}^N,">
</p>

该式对各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 通用，各时间步均用该式计算，呈现迭代计算的过程。在 ![](https://latex.codecogs.com/svg.latex?\uptau=0) 时，则会沿用上一时刻的时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=T-1) 下就算好的已知量。

### 2.4、配套的脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(0)}[\uptau]\in\mathbb{R}^N)

与 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(0)}[\uptau]\in\mathbb{R}^N) 配套的脉冲向量即为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{s}^{(0)}[\uptau]=H\left(\mathbf{u}^{(0)}[\uptau]-\theta\mathbf{1}\right)\in\mathbb{R}^N,">
</p>

其中

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?s_i^{(0)}[\uptau]=H\left(u_i^{(0)}[\uptau]-\theta\right)=\begin{cases}1,\quad{}u_i^{(0)}[\uptau]\geq\theta\\0,\quad{}u_i^{(0)}[\uptau]<\theta\end{cases}">
</p>

## 3、![](https://latex.codecogs.com/svg.latex?l=0) 层的输出

令

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]=\mathbf{s}^{(0)}[\uptau]\in\mathbb{R}^N,">
</p>

它就是 ![](https://latex.codecogs.com/svg.latex?l=0) 层的输出。

### 3.1、将计算逻辑封装为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\cdot\right)\right))

可以将 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(0)}[\uptau]) 另记为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)) ，其中 ![](https://latex.codecogs.com/svg.latex?\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)) 指的就是小脑模块所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}) 被线性转换为 ![](https://latex.codecogs.com/svg.latex?l=0) 层输入电压 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]) 的结果，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(-1\right)}[\uptau]=W^{\left(-1\right)}\cdot\mathbf{z}_{\text{mod}}+b^{\left(-1\right)}\in\mathbb{R}^N,">
</p>

此处的 ![](https://latex.codecogs.com/svg.latex?W^{\left(-1\right)}\in\mathbb{R}^{N\times{}K},b^{\left(-1\right)}\in\mathbb{R}^N) 属于可学习参数。<br>
在此基础之上，![](https://latex.codecogs.com/svg.latex?l=0) 层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) 可另记为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]=\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)\in\mathbb{R}^N.">
</p>

### 3.2、![](https://latex.codecogs.com/svg.latex?l=0) 层无法进行残差跳跃连接

由于 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}\in\mathbb{R}^K) 与 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)\in\mathbb{R}^N) 维度不匹配，因此 ![](https://latex.codecogs.com/svg.latex?l=0) 层无法进行残差跳跃连接（Residual Skip Connection）。也就是说，![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) 无法写成

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]=\mathbf{z}_{\text{mod}}+\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)\in\mathbb{R}^N">
</p>

的形式，这种写法是错误的。<br>
而且，![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}\in\mathbb{R}^K) 是连续实数向量，![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)\in\mathbb{R}^N) 是离散二元向量（每个分量只能为 ![](https://latex.codecogs.com/svg.latex?0) 或 ![](https://latex.codecogs.com/svg.latex?1) ），两者所携带信息的类型并不匹配。

## 4、总结

总而言之，![](https://latex.codecogs.com/svg.latex?l=0) 层可称为“小脑-脊髓交界面数据转换接口层”，该层接收小脑模块所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{z}_{\text{mod}}\in\mathbb{R}^K) ，并对其进行线性变换、LIF动力学演化处理，最终输出适配脉冲残差网络宽度的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]\in\mathbb{R}^N) ，为后续的更高层的计算奠定基础。
<br><br><br><br><br><br><br><br><br><br>

# 脊髓模块——脉冲残差网络的 ![](https://latex.codecogs.com/svg.latex?l=1) 残差层

## 1、将 ![](https://latex.codecogs.com/svg.latex?l=0) 层输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]\in\mathbb{R}^N) 转换为输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]\in\mathbb{R}^N)

### 1.1、输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]\in\mathbb{R}^N)

对 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]\in\mathbb{R}^N) 进行线性映射（Linear Mapping），将其映射为即将提供给首个残差层（ ![](https://latex.codecogs.com/svg.latex?l=1) 残差层）神经元的、同维度的输入电压向量，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]=W^{(0)}\cdot\mathbf{x}^{(0)}[\uptau]+b^{(0)}\in\mathbb{R}^N,">
</p>

其中 ![](https://latex.codecogs.com/svg.latex?W^{(0)}\in\mathbb{R}^{N\times{}N},b^{(0)}\in\mathbb{R}^N) 属于可学习参数。

### 1.2、![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]\in\mathbb{R}^N) 的函数性/非常数性

![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 并非保持为常数向量。相反，它是关于时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的函数向量（每一个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 单独产生一个 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) ）。这是因为，

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]=\text{LIF}\left(\text{Linear}\left(\mathbf{z}_{\text{mod}}\right)\right)\in\mathbb{R}^N">
</p>

本质上表征了LIF动力学随时间步的推进而演化的过程，包括“膜电位的衰减”、“外部电压的输入”、“阈值电位的延迟扣除”这三大并行的部分，那么，![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) 在各个时间步下的值肯定是不同的，而且存在明确的时序先后依赖关系————后一时间步的量的计算显式地依赖于前一时间步的量的已知值，只有提前算出前一时间步的量，才能开始计算后一时间步的量。<br>
进而，作为线性映射结果的 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 也就同样并非保持为常数向量。相反，它是关于时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的函数向量。

## 2、用输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]\in\mathbb{R}^N) “激发”膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(1)}[\uptau]\in\mathbb{R}^N) 及其配套的脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(1)}[\uptau]\in\mathbb{R}^N)

### 2.1、膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(1)}[\uptau]\in\mathbb{R}^N)

![](https://latex.codecogs.com/svg.latex?l=1) 残差层的膜电位向量为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{(1)}[\uptau]=\gamma\cdot{}\mathbf{u}^{(1)}\left[\uptau-1\right]+\mathbf{U}^{(0)}[\uptau]-\theta\mathbf{s}^{(1)}\left[\uptau-1\right]\in\mathbb{R}^N,">
</p>

该式对各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 通用，各时间步均用该式计算，呈现迭代计算的过程。在 ![](https://latex.codecogs.com/svg.latex?\uptau=0) 时，则会沿用上一时刻的时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=T-1) 下就算好的已知量。

### 2.2、配套的脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(1)}[\uptau]\in\mathbb{R}^N)

与 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(1)}[\uptau]\in\mathbb{R}^N) 配套的脉冲向量即为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{s}^{(1)}[\uptau]=H\left(\mathbf{u}^{(1)}[\uptau]-\theta\mathbf{1}\right)\in\mathbb{R}^N,">
</p>

其中

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?s_i^{(1)}[\uptau]=H\left(u_i^{(1)}[\uptau]-\theta\right)=\begin{cases}1,\quad{}u_i^{(1)}[\uptau]\geq\theta\\0,\quad{}u_i^{(1)}[\uptau]<\theta\end{cases}">
</p>

## 3、![](https://latex.codecogs.com/svg.latex?l=1) 残差层的输出

令

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(1)}[\uptau]=\mathbf{x}^{(0)}[\uptau]+\mathbf{s}^{(1)}[\uptau]\in\mathbb{R}^N,">
</p>

它就是 ![](https://latex.codecogs.com/svg.latex?l=1) 残差层的输出，是残差跳跃连接（残差相加）的结果。

### 3.1、来自笔者的提醒

尤其注意！<br>
1、低一层残差层的输出是高一层残差层的输入。<br>
2、脉冲残差网络的残差层的输出是离散二元性的脉冲向量（的残差相加量），而非连续实数性的膜电位向量。<br>
3、低一层残差层向高一层残差层传递的向量是脉冲向量，而非膜电位向量。<br>
4、膜电位向量仅仅是残差层内部的产物。<br>
5、残差层输出并向上传递的脉冲向量仅携带关于膜电位向量的很小一部分的信息（即“膜电位超过某个阈值/膜电位不超过某个阈值”这种单一的信息），那些只有连续实数形式的向量才能表达的信息（占大部分）则完全不携带。这部分信息仍然只由连续实数性的膜电位向量携带。这一事实也为 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层的设计提供了一定依据。

### 3.2、将计算逻辑封装为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\cdot\right)\right))

将 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(1)}[\uptau]) 另记为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{(0)}[\uptau]\right)\right)) ，其中 ![](https://latex.codecogs.com/svg.latex?\text{Linear}\left(\mathbf{x}^{(0)}[\uptau]\right)) 指的就是 ![](https://latex.codecogs.com/svg.latex?l=0) 层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) 被线性映射为 ![](https://latex.codecogs.com/svg.latex?l=1) 残差层输入电压 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]) 的结果，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{(0)}[\uptau]=W^{(0)}\cdot\mathbf{x}^{(0)}[\uptau]+b^{(0)}\in\mathbb{R}^N,">
</p>

此处的 ![](https://latex.codecogs.com/svg.latex?W^{(0)}\in\mathbb{R}^{N\times{}N},b^{(0)}\in\mathbb{R}^N) 属于可学习参数。<br>
在此基础之上，![](https://latex.codecogs.com/svg.latex?l=1) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(1)}[\uptau]) 可另记为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(1)}[\uptau]=\mathbf{x}^{(0)}[\uptau]+\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{(0)}[\uptau]\right)\right)\in\mathbb{R}^N.">
</p>

### 3.3、![](https://latex.codecogs.com/svg.latex?l=1) 残差层可以且应该进行残差跳跃连接

由于 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]\in\mathbb{R}^N) 与 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{(0)}[\uptau]\right)\right)\in\mathbb{R}^N) 维度匹配，因此 ![](https://latex.codecogs.com/svg.latex?l=1) 残差层可以且应该进行残差跳跃连接。<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]\in\mathbb{R}^N) 是离散二元向量，![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{(0)}[\uptau]\right)\right)\in\mathbb{R}^N) 同样是离散二元向量，两者所携带信息的类型是匹配的。

## 4、总结

总而言之，![](https://latex.codecogs.com/svg.latex?l=1) 残差层作为网络中的首个残差层，接收 ![](https://latex.codecogs.com/svg.latex?l=0) 层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]\in\mathbb{R}^N) ，并对其进行线性映射、LIF动力学演化、残差相加处理，最终输出同维度的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(1)}[\uptau]\in\mathbb{R}^N) 。<br>
![](https://latex.codecogs.com/svg.latex?l=1) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(1)}[\uptau]\in\mathbb{R}^N) 即将被传递给 ![](https://latex.codecogs.com/svg.latex?l=2) 残差层。
<br><br><br><br><br><br><br><br><br><br>

# 脊髓模块——脉冲残差网络的 ![](https://latex.codecogs.com/svg.latex?l=2) 至 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层

设定脉冲残差网络的残差层的层数为 ![](https://latex.codecogs.com/svg.latex?L) ，各残差层分别为 ![](https://latex.codecogs.com/svg.latex?l=1,l=2,\cdots,l=L) 残差层。<br>
应注意，![](https://latex.codecogs.com/svg.latex?l=0) 层是数据转换接口层，属于脉冲残差网络的组成部分，但严格意义上不能被称作“残差层”（因为该层无法进行残差相加）。![](https://latex.codecogs.com/svg.latex?l=L+1) 层是运动解码层，同样属于脉冲残差网络的组成部分，但严格意义上也不能称作“残差层”。<br>
更高的 ![](https://latex.codecogs.com/svg.latex?l=2) 至 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层的行为与 ![](https://latex.codecogs.com/svg.latex?l=1) 残差层的行为除开各层参数不同、各层实际处理的量不同（存在先后依赖关系，即后一层的输入是前一层的输出）以外，没有任何区别，均遵循完全相同的计算流程。<br>
在后文中，用 ![](https://latex.codecogs.com/svg.latex?l) 指代当前残差层，满足

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?l\in\{2,\cdots,L\},">
</p>

同时用 ![](https://latex.codecogs.com/svg.latex?l-1) 指代前一层残差层，在此基础之上，对 ![](https://latex.codecogs.com/svg.latex?l=2) 至 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层的行为进行一般性归纳。

## 1、将 ![](https://latex.codecogs.com/svg.latex?l-1) 残差层输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) 转换为输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N)

### 1.1、输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N)

对 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) 进行线性映射，将其映射为即将提供给 ![](https://latex.codecogs.com/svg.latex?l) 残差层神经元的、同维度的输入电压向量，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]=W^{\left(l-1\right)}\cdot\mathbf{x}^{\left(l-1\right)}[\uptau]+b^{\left(l-1\right)}\in\mathbb{R}^N,">
</p>

其中 ![](https://latex.codecogs.com/svg.latex?W^{\left(l-1\right)}\in\mathbb{R}^{N\times{}N},b^{\left(l-1\right)}\in\mathbb{R}^N) 属于可学习参数。

### 1.2、![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) 的函数性/非常数性

![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 并非保持为常数向量。相反，它是关于时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的函数向量（每一个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 单独产生一个 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]) ）。其原因不再赘述。<br>
作为线性映射结果的 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 也就同样并非保持为常数向量。相反，它是关于时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的函数向量。

## 2、用输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) “激发”膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(l)}[\uptau]\in\mathbb{R}^N) 及其配套的脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]\in\mathbb{R}^N)

### 2.1、膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(l)}[\uptau]\in\mathbb{R}^N)

![](https://latex.codecogs.com/svg.latex?l) 残差层的膜电位向量为
...
### 2.2、配套的脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]\in\mathbb{R}^N)

与 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{(l)}[\uptau]\in\mathbb{R}^N) 配套的脉冲向量即为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]=H\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right)\in\mathbb{R}^N,">
</p>
...
## 3、![](https://latex.codecogs.com/svg.latex?l) 残差层的输出

令

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(l)}[\uptau]=\mathbf{x}^{\left(l-1\right)}[\uptau]+\mathbf{s}^{(l)}[\uptau]\in\mathbb{R}^N,">
</p>

它就是 ![](https://latex.codecogs.com/svg.latex?l) 残差层的输出，是残差跳跃连接（残差相加）的结果。

### 3.1、将计算逻辑封装为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\cdot\right)\right))

将 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]) 另记为 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{\left(l-1\right)}[\uptau]\right)\right)) ，其中 ![](https://latex.codecogs.com/svg.latex?\text{Linear}\left(\mathbf{x}^{\left(l-1\right)}[\uptau]\right)) 指的就是 ![](https://latex.codecogs.com/svg.latex?l-1) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]) 被线性映射为 ![](https://latex.codecogs.com/svg.latex?l) 残差层输入电压 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]) 的结果，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{\left(l-1\right)}[\uptau]=W^{\left(l-1\right)}\cdot\mathbf{x}^{\left(l-1\right)}[\uptau]+b^{\left(l-1\right)}\in\mathbb{R}^N,">
</p>

此处的 ![](https://latex.codecogs.com/svg.latex?W^{\left(l-1\right)}\in\mathbb{R}^{N\times{}N},b^{\left(l-1\right)}\in\mathbb{R}^N) 属于可学习参数。<br>
在此基础之上，![](https://latex.codecogs.com/svg.latex?l) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(l)}[\uptau]) 可另记为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{x}^{(l)}[\uptau]=\mathbf{x}^{\left(l-1\right)}[\uptau]+\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{\left(l-1\right)}[\uptau]\right)\right)\in\mathbb{R}^N.">
</p>

### 3.2、![](https://latex.codecogs.com/svg.latex?l) 残差层可以且应该进行残差跳跃连接

由于 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) 与 ![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{\left(l-1\right)}[\uptau]\right)\right)\in\mathbb{R}^N) 维度匹配，因此 ![](https://latex.codecogs.com/svg.latex?l) 残差层可以且应该进行残差跳跃连接。<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) 是离散二元向量，![](https://latex.codecogs.com/svg.latex?\text{LIF}\left(\text{Linear}\left(\mathbf{x}^{\left(l-1\right)}[\uptau]\right)\right)\in\mathbb{R}^N) 同样是离散二元向量，两者所携带信息的类型是匹配的。

## 4、总结

总而言之，![](https://latex.codecogs.com/svg.latex?l) 残差层（满足 ![](https://latex.codecogs.com/svg.latex?l\in2,\cdots,L) ）接收 ![](https://latex.codecogs.com/svg.latex?l-1) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]\in\mathbb{R}^N) ，并对其进行线性映射、LIF动力学演化、残差相加处理，最终输出同维度的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(l)}[\uptau]\in\mathbb{R}^N) 。<br>
![](https://latex.codecogs.com/svg.latex?l) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(l)}[\uptau]\in\mathbb{R}^N) 即将被传递给 ![](https://latex.codecogs.com/svg.latex?l+1) 残差层。<br>
具体而言：<br>
![](https://latex.codecogs.com/svg.latex?l=2) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(2)}[\uptau]\in\mathbb{R}^N) 即将被传递给 ![](https://latex.codecogs.com/svg.latex?l=3) 残差层；<br>
![](https://latex.codecogs.com/svg.latex?l=3) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(3)}[\uptau]\in\mathbb{R}^N) 即将被传递给 ![](https://latex.codecogs.com/svg.latex?l=4) 残差层；<br>
![](https://latex.codecogs.com/svg.latex?\cdots) <br>
![](https://latex.codecogs.com/svg.latex?l=L-1) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(L-1\right)}[\uptau]\in\mathbb{R}^N) 即将被传递给 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层；<br>
![](https://latex.codecogs.com/svg.latex?l=L) 残差层所输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(L)}[\uptau]\in\mathbb{R}^N) 即将被传递给 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层。<br>
针对 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层的说明将在下一章当中展开。
<br><br><br><br><br><br><br><br><br><br>

# 脊髓模块——脉冲残差网络的 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层

![](https://latex.codecogs.com/svg.latex?l=1) 至 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层也可称为“隐藏中间层”（hidden interlayers）。![](https://latex.codecogs.com/svg.latex?l=0) 数据转换接口层的行为与隐藏中间层的行为存在明显的区别，![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层的行为与隐藏中间层的行为同样存在明显的区别。

## 1、将 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层输出的 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(L)}[\uptau]\in\mathbb{R}^N) 转换为输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]\in\mathbb{R}^M)

### 1.1、输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]\in\mathbb{R}^M)

对 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(L)}[\uptau]\in\mathbb{R}^N) 进行线性变换，将其变换为即将提供给 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层神经元的输入电压向量，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]=W^{(L)}\cdot\mathbf{x}^{(L)}[\uptau]+b^{(L)}\in\mathbb{R}^M,">
</p>

其中 ![](https://latex.codecogs.com/svg.latex?W^{(L)}\in\mathbb{R}^{M\times{}N},b^{(L)}\in\mathbb{R}^M) 属于可学习参数，![](https://latex.codecogs.com/svg.latex?M) 表示运动解码层（Motor Decoding Layer）的神经元个数。![](https://latex.codecogs.com/svg.latex?M) 可以选择等于 ![](https://latex.codecogs.com/svg.latex?N) ，但由于 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层作为网络中的最后一层，并不存在残差相加的需求，因此，![](https://latex.codecogs.com/svg.latex?M) 并非必须等于 ![](https://latex.codecogs.com/svg.latex?N) 。

### 1.2、![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]\in\mathbb{R}^M) 的函数性/非常数性

![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]) 是关于时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的函数向量，其原因不再赘述。

## 2、用输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]\in\mathbb{R}^M) “激发并无衰减累积”膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M)，同时并不需要脉冲向量

### 2.1、膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M)

![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层的膜电位向量为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]=\mathbf{u}^{\left(L+1\right)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\in\mathbb{R}^M,">
</p>

尤其注意！右侧没有 ![](https://latex.codecogs.com/svg.latex?-\theta\mathbf{s}^{\left(L+1\right)}\left[\uptau-1\right]) 这个扣除项，右侧的 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}\left[\uptau-1\right]) 项没有 ![](https://latex.codecogs.com/svg.latex?\gamma) 这个衰减因子系数，它们是被刻意取消的。在后文的第4节当中，将详细论述这种做法的设计考量。<br>
该式对各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 通用，各时间步均用该式计算，呈现迭代计算的过程。在 ![](https://latex.codecogs.com/svg.latex?\uptau=0) 时，则会沿用上一时刻的时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=T-1) 下就算好的已知量。

### 2.2、![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层无需脉冲向量

由于迭代式当中 ![](https://latex.codecogs.com/svg.latex?-\theta\mathbf{s}^{\left(L+1\right)}\left[\uptau-1\right]) 这个扣除项被取消了，所以当前时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 下也就同样无需再为后一个时间步计算脉冲向量。

## 3、![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层的输出——机器人的物理运动指令向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]\in\mathbb{R}^A)

令

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]=W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^A,">
</p>

它就是 ![](https://latex.codecogs.com/svg.latex?l=L+1) 运动解码层所输出的机器人物理运动指令向量，它表征了机器人在相应时间步下所应该执行的某种具体物理动作。其中，![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\in\mathbb{R}^{A\times{}M}) ，![](https://latex.codecogs.com/svg.latex?A) 表示物理运动指令向量的维度数目。

## 4、“采用 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]) 而非 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{s}^{\left(L+1\right)}[\uptau]) ”、“删除 ![](https://latex.codecogs.com/svg.latex?\gamma) 和 ![](https://latex.codecogs.com/svg.latex?-\theta\mathbf{s}^{\left(L+1\right)}\left[\uptau-1\right]) ”的设计考量

理想情况下，机器人的运动轨迹（motor trajectories）在空间上是平滑的（smooth）、连续的（continuous），其动作序列（action sequence）在时间上是连贯的（consistent）。换句话说，运动学上的急动度（jerk，即加速度 ![](https://latex.codecogs.com/svg.latex?a) 对时间 ![](https://latex.codecogs.com/svg.latex?t) 的一阶导数 ![](https://latex.codecogs.com/svg.latex?da/dt) ）是很小的。<br>
在深度学习框架中，这其实就意味着，表征物理动作的物理运动指令向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]\in\mathbb{R}^A) 随时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的变化是平缓的，不会发生值的突变。

### 4.1、为什么不能采用 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{s}^{\left(L+1\right)}[\uptau])

![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M) 是一个离散二元向量，其分量要么是 ![](https://latex.codecogs.com/svg.latex?0) ，要么是 ![](https://latex.codecogs.com/svg.latex?1) ，这在数学上就会带来一个后果：<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M) 的取值是有限的，只有 ![](https://latex.codecogs.com/svg.latex?2^M) 种，因此 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{s}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M) 的取值也就相应地只有 ![](https://latex.codecogs.com/svg.latex?2^M) 种，并且这些取值是离散的，从一个值变化到另一个值的过程完全不具备类似于“线性插值”那样的连续性。相反，从一个值变化到另一个值的过程是切换式的，这种过程天然具备突变性。<br>
倘若 ![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]) 采用 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{s}^{\left(L+1\right)}[\uptau]) ，可以料想，机器人的运动学急动度将极其之大，其动作将时不时发生突变。这不仅会在短时间内就对机器人零件造成严重磨损，还完全可能引发严重的安全事故。<br>
无论如何，![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]) 都绝对不能直接基于脉冲向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{\left(L+1\right)}[\uptau]) 产生，这是非常危险的。

### 4.2、![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]) 的关键优点——函数平滑性

与 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M) 不同，![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]\in\mathbb{R}^M) 是一个连续实数向量，并且被专门设计为一种在时间上进行积分的量，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\begin{align*}\mathbf{u}^{\left(L+1\right)}[\uptau]&=\mathbf{u}^{\left(L+1\right)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&=\mathbf{u}^{\left(L+1\right)}\left[\uptau-2\right]+\mathbf{U}^{(L)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&=\mathbf{u}^{\left(L+1\right)}\left[\uptau-3\right]+\mathbf{U}^{(L)}\left[\uptau-2\right]+\mathbf{U}^{(L)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&\cdots\\&=\mathbf{u}^{\left(L+1\right)}\left[-1\right]+\mathbf{U}^{(L)}[0]+\cdots+\mathbf{U}^{(L)}\left[\uptau-2\right]+\mathbf{U}^{(L)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]\\&=\mathbf{u}^{\left(L+1\right)}\left[T-1\right]_{t-1\sim{}t}+\sum_{\uptau^\prime=0}^{\uptau}\mathbf{U}^{(L)}\left[\uptau^\prime\right]\in\mathbb{R}^M,\end{align*}">
</p>

其中

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}\left[-1\right]=\mathbf{u}^{\left(L+1\right)}\left[T-1\right]_{t-1\sim{}t}">
</p>

是上一时刻的时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=T-1) 下就算好的已知量。<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]) 同样是一个连续实数向量，其取值不存在离散限制，可以由网络根据需要，通过自行调整网络参数进行自由表达，展现出本质上的、连续函数所具备的那种平滑性。因而，对本质上具备函数平滑性的输入电压向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]) 进行持续累积的膜电位向量 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]) 也就天然具备函数平滑性。<br>
对于原始迭代式

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]=\gamma\cdot\mathbf{u}^{\left(L+1\right)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau]-\theta\mathbf{s}^{\left(L+1\right)}\left[\uptau-1\right],">
</p>

删除右侧的 ![](https://latex.codecogs.com/svg.latex?\gamma) 和 ![](https://latex.codecogs.com/svg.latex?-\theta\mathbf{s}^{\left(L+1\right)}\left[\uptau-1\right]) ，使式子变成迭代积分的形式，即

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]=\mathbf{u}^{\left(L+1\right)}\left[\uptau-1\right]+\mathbf{U}^{(L)}[\uptau],">
</p>

正是为了让 ![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]) 能够从 ![](https://latex.codecogs.com/svg.latex?\mathbf{U}^{(L)}[\uptau]) 身上自然而然地获得这种函数平滑性，同时排除与这一目的无关的因素。这种做法其实就是“连续积分协议”（Continuous Integration Protocol）这一概念的实例化，原论文作者团队标注了与“连续积分协议”有关的文献。<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{u}^{\left(L+1\right)}[\uptau]) 具备函数平滑性，那么 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]) 也就相应地具备函数平滑性。具备函数平滑性的 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]) 完美适配机器人的连续运动控制。<br>
![](https://latex.codecogs.com/svg.latex?\mathbf{a}_t[\uptau]) 采用 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\cdot\mathbf{u}^{\left(L+1\right)}[\uptau]) 后，机器人的运动学急动度可长期保持低位水平，机器人的动作将表现出良好的连贯性和协调性。

## 5、原文引用

原论文作者团队指出：<br>
“将脉冲神经网络应用于机器人技术的一个关键挑战在于弥合离散、随机的脉冲与关节驱动的连续域之间的鸿沟。我们通过在输出层实现连续积分协议来解决这一问题。与经历快速复极化（重置）的隐藏中间神经元不同，输出运动神经元被配置为非重置积分器（non-resetting integrators）。它们在模拟窗口上持续累积突触输入而不释放其电位。”<br>
“该机制服务于两个生理功能：<br>
1、时间平滑：积分过程充当自然的低通滤波器（low-pass filter），将高频脉冲序列转换为平滑、连续的运动轨迹。这反映了生物上的颤搐叠加（twitch summation）过程，即肌肉将神经驱动积分为连续力，滤除神经噪声。<br>
2、运动学连贯性：通过维持电压状态，网络自然地在输出动作序列中强制时序连贯性，减少运动学急动度，并确保控制步之间的流畅过渡。”
<br><br><br><br><br><br><br><br><br><br>
...
### 1.1、脉冲生成函数 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]=H\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right)) 是一种Heaviside阶跃函数

Heaviside阶跃函数（Heaviside Step Function）是一种广义函数，定义为

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?H(x)=\begin{cases}1,\quad{}x>0\\0,\quad{}x<0\end{cases}">
</p>

![](https://latex.codecogs.com/svg.latex?H(x)) 在 ![](https://latex.codecogs.com/svg.latex?x=0) 处的取值可以是 ![](https://latex.codecogs.com/svg.latex?1)（右连续约定），可以是 ![](https://latex.codecogs.com/svg.latex?0)（左连续约定），可以是 ![](https://latex.codecogs.com/svg.latex?1/2)（对称约定），也可以不定义（不理睬）。<br>
对于脉冲生成函数

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]=H\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right),">
</p>

其中

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?s_i^{(l)}[\uptau]=H\left(u_i^{(l)}[\uptau]-\theta\right)=\begin{cases}1,\quad{}u_i^{(l)}[\uptau]-\theta\geq0,u_i^{(l)}[\uptau]\geq\theta\\0,\quad{}u_i^{(l)}[\uptau]-\theta<0,u_i^{(l)}[\uptau]<\theta\end{cases}">
</p>

很明显，它就是一种右连续约定的、向量形式的Heaviside阶跃函数，各分量的值被独立地判定为 ![](https://latex.codecogs.com/svg.latex?1) 或 ![](https://latex.codecogs.com/svg.latex?0) 。

### 1.2、梯度链的“断裂”

由于这类函数在原点处不可导，所以这类函数具备不可微性（non-differentiability），也就是说，这类函数对自变量的导数是不可解的。在深度学习的梯度下降（Gradient Descent）训练框架中，这意味着从损失函数到部分可学习参数（笔者将其称为“下游可学习参数”）的梯度链（梯度流）将在该函数处发生“断裂”，直接使得这部分下游参数的优化变得不可行，端到端（end-to-end）的梯度下降优化也就随之变得不可行。<br>
...
<p align="center">
<img src="https://latex.codecogs.com/svg.latex?L=f\left(\widetilde{a}_t[\uptau],a_t[\uptau]\right),">
</p>

其中 ![](https://latex.codecogs.com/svg.latex?\widetilde{a}_t[\uptau]) 表征人类专家所示范的动作。<br>
针对一般形式
...
尽管从损失函数 ![](https://latex.codecogs.com/svg.latex?L) 到三个下游参数 ![](https://latex.codecogs.com/svg.latex?W_{\text{out}}\in\mathbb{R}^{A\times{}M},W^{(L)}\in\mathbb{R}^{M\times{}N},b^{(L)}\in\mathbb{R}^M) 的各条梯度链均不存在Heaviside阶跃函数成分，也就不存在“断裂”的现象，但是，端到端的梯度下降训练框架要求网络中的所有参数同时接受优化，而非仅仅个别参数接受优化。

## 2、解决方法

### 2.1、用可微函数替代不可微函数

梯度链断裂的根本原因在于脉冲生成函数 ![](https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]) 被设置成不可微的 ![](https://latex.codecogs.com/svg.latex?H\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right)) 函数，而 ![](https://latex.codecogs.com/svg.latex?H\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right)) 函数不可微的根本原因在于它作为Heaviside阶跃函数，在原点处不可导。<br>
由此出发，一个自然的想法就是用一个在原点处可导的、具备可微性的、形状近似的“S”型函数来替代在原点处不可导的 ![](https://latex.codecogs.com/svg.latex?H\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right)) ，承担脉冲生成的职责。<br>
这种解决方法属于“代理梯度方法”（surrogate gradient method）。
...
<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\mathbf{s}^{(l)}[\uptau]=\sigma\left(\mathbf{u}^{(l)}[\uptau]-\theta\mathbf{1}\right),">
</p>

其中

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?s_i^{(l)}[\uptau]=\sigma\left(u_i^{(l)}[\uptau]-\theta\right)=\frac{u_i^{(l)}[\uptau]-\theta}{1+\left\lvert{}u_i^{(l)}[\uptau]-\theta\right\rvert},">
</p>

此处的

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\sigma(x)=\frac{x}{1+\lvert{}x\rvert}">
</p>

就是Fast Sigmoid函数。它是单调递增的、关于原点对称的函数，其函数值范围是 ![](https://latex.codecogs.com/svg.latex?\left(-1,1\right)) 。

### 2.3、Sigmoid函数与Fast Sigmoid函数的比较

相比原始的Sigmoid函数

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\sigma(x)=\frac{1}{1+e^{-x}}\in\left(0,1\right),">
</p>

Fast Sigmoid函数将分母中的指数运算 ![](https://latex.codecogs.com/svg.latex?e^{-x}) 替换为计算成本更小、计算速度更快的绝对值运算 ![](https://latex.codecogs.com/svg.latex?\lvert{}x\rvert) 。尽管分子由 ![](https://latex.codecogs.com/svg.latex?1) 替换成了 ![](https://latex.codecogs.com/svg.latex?x) ，但依然没有经过需要一定开销的特殊运算（单独的 ![](https://latex.codecogs.com/svg.latex?x) 本身根本就不构成任何实际运算）。

### 2.4、Heaviside阶跃函数与Fast Sigmoid函数的相同点

Heaviside阶跃函数

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?H(x)=\begin{cases}1,\quad{}x\geq0\\0,\quad{}x<0\end{cases}">
</p>
...
在形状上都类似于字母“S”。<br>
当 ![](https://latex.codecogs.com/svg.latex?x) 为负数时，如果用Heaviside阶跃函数对它进行激活，那么它会变成 ![](https://latex.codecogs.com/svg.latex?0) ，如果用Fast Sigmoid函数对它进行激活，那么它会变成一个位于 ![](https://latex.codecogs.com/svg.latex?\left(-1,0\right)) 区间内的负数。这两种激活结果其实都表达了同一种网络行为语义————对 ![](https://latex.codecogs.com/svg.latex?x) 原本所包含信息的抑制。<br>
当 ![](https://latex.codecogs.com/svg.latex?x) 为正数时，如果用Heaviside阶跃函数对它进行激活，那么它会变成 ![](https://latex.codecogs.com/svg.latex?1) ，如果用Fast Sigmoid函数对它进行激活，那么它会变成一个位于 ![](https://latex.codecogs.com/svg.latex?\left(0,1\right)) 区间内的正数。这两种激活结果其实都表达了同一种网络行为语义————对 ![](https://latex.codecogs.com/svg.latex?x) 原本所包含信息的增强。

### 2.5、Heaviside阶跃函数与Fast Sigmoid函数的关键区别

Heaviside阶跃函数在原点处不可导，而Fast Sigmoid函数在原点处是平滑可导的。Fast Sigmoid函数在原点处的导数等于 ![](https://latex.codecogs.com/svg.latex?1) 。<br>
Heaviside阶跃函数在原点处的值只能被人为规定，而且无法做到左右同时连续。但Fast Sigmoid函数在原点处的值自然为 ![](https://latex.codecogs.com/svg.latex?0) ，而且自然地实现左右同时连续。

## 3、原文引用

原论文作者团队指出：<br>
“为了克服脉冲生成函数 ![](https://latex.codecogs.com/svg.latex?s=\Theta\left(u-\vartheta\right)) 的不可微性，我们采用一种替代梯度方法进行端到端训练。在反向传播期间，Heaviside阶跃函数由Fast Sigmoid函数

<p align="center">
<img src="https://latex.codecogs.com/svg.latex?\sigma(x)=\frac{x}{1+\lvert{}x\rvert}">
</p>

进行近似，从而允许梯度从连续动作空间反向流经离散脊髓层，直至小脑和皮层模块。”
