...
更高的 ![](https://latex.codecogs.com/svg.latex?l=2) 至 ![](https://latex.codecogs.com/svg.latex?l=L) 残差层的行为与 ![](https://latex.codecogs.com/svg.latex?l=1) 残差层的行为除开各层实际处理的量不同，存在先后依赖关系（后一层的输入是前一层的输出）以外，没有任何区别，均遵循完全相同的计算流程。<br>
...
![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{\left(l-1\right)}[\uptau]) 对时刻 ![](https://latex.codecogs.com/svg.latex?t) 与 ![](https://latex.codecogs.com/svg.latex?t+1) 之间的各个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau=0,1,2,\cdots,T-1) 并非保持为常数向量。相反，它是关于时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 的函数向量（每一个时间步 ![](https://latex.codecogs.com/svg.latex?\uptau) 单独产生一个 ![](https://latex.codecogs.com/svg.latex?\mathbf{x}^{(0)}[\uptau]) ）。其原因不再赘述。<br>
